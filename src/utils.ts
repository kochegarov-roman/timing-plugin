...
export function getEventsByWeekDay(_eventsByWeekDay: Record<string, IEvent[]>, _events: IEvent[]) : [Record<string, IEvent[]> , IEvent[]] {
  ...
  _events
    ...
    .forEach((ev) => {
      ...
      _eventsByWeekDay[weekDay] = _eventsByWeekDay[weekDay] ? [..._eventsByWeekDay[weekDay], ev] : [ev];
    });
...
export const findActiveHour = (dividerHours: number, hours: string[]): string => {
  ...
  for (const hour of hours) {
    ...
    if (!hours.includes(activeHour) && curInd > indStart && curInd < hoursFull.indexOf(activeHour)) {
