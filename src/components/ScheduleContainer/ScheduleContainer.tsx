...
const ScheduleContainer: FC<ScheduleContainerProps> = ({ appInitData }) => {
  ...
  const [eventsByWeekDay, setEventsByWeekDay] = useState<
    Record<string, IEvent[]>
  >({});

  const [allStartEndHoursStream, setAllStartEndHoursStream] = useState<
    string[]
  >([]);
  ...
  const hoursContainerWidth =
    document.getElementById(appInitData.widthIdSelector)?.offsetWidth || 500;

  function onResize() {
    const hoursResult = generateHours(
      allStartEndHoursStream,
      isCurrentWeek && appInitData.performerIsLive === "true",
    );
    let divider = 1;
