...
const Events: FC<IEventsProps> = ({ events, selectedEventKey, handleSelectEvent, appInitInfo }) => {
    return (
        <>
            {events.map((event) => (
                <div
                ...
                >
                    <div
                        className={
                            ...
                            (selectedEventKey === event.contentId + event.type ? ' selected-event' : '') +
                    ...
                    >
                        <div className="stream-schedule-segment--fade">
                            <div className="text stream-schedule-segment--text">
                                <p itemProp="name">{event.title === 'null' ? appInitInfo.performerScreenName : event.title}</p>
                            </div>
                            <div className="text stream-schedule-segment--info">
                                <p className="stream-schedule-segment--text" itemProp="description">{event.description}</p>
                                {event.isLive && (
                                    <>
                            ...
                            <div className="hidden"><a href={event.srcUrl} itemProp="url"></a></div>
                            <div style={{display: 'flex', marginBottom: 0}}>
                                <hr/>
                                <div className="stream-schedule-segment--text"
                                     itemProp="startDate">{event.isLive ? 'Live Now' : event.date}</div>

                                <div className="hidden"
                                     itemProp="endDate">{event.dateEnd}</div>
                            </div>
                        </div>

                        {event.previewImgUrl && event.width && event.width > 140 && (
                            <div className="preview">
                                <img src={event.previewImgUrl} alt={event.description} itemProp="image"/>
